...
export default function SellerManager() {
  let user = JSON.parse(localStorage.getItem('token'))
    ...
    useEffect(()=>{
        if(axios){
            axios.get(`/bill/selectByUser/${user.id}?status=pending`)
            .then(res=> setOrderList(res.data.filter(item=> item.status==='pending')))
    ...
    const accept = (id)=>{
        ...
        axios.put(`/bill/updateStatus/${id}?status=active`)
        .then(()=> enqueueSnackbar('Đã xác nhận giao hàng', {variant:'success'}))
        .catch(()=> enqueueSnackbar('Xác nhận giao hàng thất bại', {variant:'error'}))
    }
    const deleteAll = () =>{
    ...
    const remove = (id) =>{
      ...
      .then(()=> enqueueSnackbar('Đã bỏ đơn đặt của người mua', {variant:'success'}))
      .catch(()=> enqueueSnackbar('Bỏ đơn đặt thất bại', {variant:'error'}))
    }
  return (
    <BgUser>
                ...
                {orderList && orderList?.length > 0 ? (
                  orderList.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <button type="button" onClick={()=>accept(item.idBill)} className="btn btn-info me-2">
                            Giao hàng
                        </button>
                        <button
                          type="button"
